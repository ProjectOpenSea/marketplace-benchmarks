...
contract InitializableHarness is Initializable {
    ...
    function nested_init_reinit(uint8 m) public initializer {
        reinitialize(m);
    }

    function nested_reinit_init(uint8 n) public reinitializer(n) {
        initialize();
    }

    function nested_reinit_reinit(uint8 n, uint8 m) public reinitializer(n) {
        reinitialize(m);
    }
